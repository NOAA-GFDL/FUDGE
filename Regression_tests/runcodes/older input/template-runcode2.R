...
#------------ historical predictor(s)----------# 
	hist.file.start.year_1 <- [HSTART] 
	hist.file.end.year_1 <- [HEND]
        hist.train.start.year_1 <- [HSTART]
	hist.train.end.year_1 <- [HEND] 
	hist.scenario_1 <- 'historical_[HIST_RIP]'
	hist.nyrtot_1 <- (hist.train.end.year_1 - hist.train.start.year_1) + 1
	hist.model_1 <- 'MPI-ESM-LR' 
	hist.freq_1 <- 'day' 
	hist.indir_1 <- '/archive/esd/PROJECTS/DOWNSCALING///GCM_DATA/CMIP5//MPI-ESM-LR/historical//atmos/day/[HIST_RIP]/v20111006/[VAR]/[GRID]/OneD/' 
	hist.time.window <- '/archive/esd/PROJECTS/DOWNSCALING/3ToThe5th/masks/timemasks/maskdays_[TMASK_FREQUENCY]_[HIST_DATE_RANGE].nc' 
#------------ future predictor(s) -------------# 
	fut.file.start.year_1 <- [FSTART] 
	fut.file.end.year_1 <- [FEND] 
        fut.train.start.year_1 <- [FSTART] 
        fut.train.end.year_1 <- [FEND] 
	fut.scenario_1 <- '[FUT_SCENARIO]_[FUT_RIP]'
	fut.nyrtot_1 <- (fut.train.end.year_1 - fut.train.start.year_1) + 1
	fut.model_1 <- 'MPI-ESM-LR' 
	fut.freq_1 <- 'day' 
	fut.indir_1 <- '/archive/esd/PROJECTS/DOWNSCALING///GCM_DATA/CMIP5//[FUT_GCM]/[FUT_SCENARIO]/day/atmos/day/[FUT_RIP]/[FUT_DATA_VERSION]/pr/[GRID]/OneD/'
	fut.time.window <- '/archive/esd/PROJECTS/DOWNSCALING/3ToThe5th/masks/timemasks/maskdays_[TMASK_FREQUENCY]_[FUT_DATE_RANGE].nc'
        fut.time.trim.mask <- 'na'
#------------- target -------------------------# 
	target.file.start.year_1 <- [HSTART] 
	target.file.end.year_1 <- [HEND] 
        target.train.start.year_1 <- [HSTART] 
        target.train.end.year_1 <- [HEND] 
	...
	target.freq_1 <- 'day' 
        target.indir_1 <- '/archive/esd/PROJECTS/DOWNSCALING///OBS_DATA/GRIDDED_OBS//livneh/historical//atmos/day/r0i0p0/v1p2/pr/[GRID]/OneD/'
	target.time.window <- '/archive/esd/PROJECTS/DOWNSCALING/3ToThe5th/masks/timemasks/maskdays_[TMASK_FREQUENCY]_[HIST_DATE_RANGE].nc'
#------------- method name k-fold specs-----------------------#
        ds.method <- '[DS_METHOD]' 
	ds.experiment <- 'RR[VAR_ID]p1-[DS_METHOD]-B38ap-[TESTNAME]L01K00' 
	k.fold <- 0 
	#pr_opts=list(pr_threshold_in='us_trace',pr_freqadj_in='off',pr_conserve_in='on',pr_threshold_out='us_trace',pr_conserve_out='on')
	pr_opts=[PR_OPTS]
#-------------- output -----------------------#
	output.dir <- 'OUTDIR'
	mask.output.dir <- 'OUTDIR' 
#-------------  custom -----------------------#
