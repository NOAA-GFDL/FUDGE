...
if ($mode == 'runcode') then
	...
	python $BASEDIR/bin/expergen.py -i $runcode >>& $logfile
	sleep 30
	set scriptdir = `tail $logfile | grep -oP "RunScripts will be saved under: \K.*"`
	set runscript = `find $scriptdir/runscript/` #Should only ever be one file in this 
	echo "tcsh $runscript" >>& $logfile
	tcsh $runscript >>& $logfile
	sleep 30
	...
	set dsout = `tail $logfile -n 50 | grep -oP "Final Downscaled output file location:\K.*"` #as opposed to version without /vftmp
	echo "downscaled output: $dsout"
	#set dscmp = `tail $logfile -n 10 | grep -oP "Final Downscaled output file location:\K.*"`
	#if (! -e $dsout) then
	...
	#set outdir = dirname $logfile #Not sure this is neccessary. Try it and see if it braks?
	set temp_file  = "$newdir/fudgelist.tmp"
	#TODO: Set up the sed script replacement for the XMLs
	cp $runcode $newdir/runcode
	sed -i "s@OUTPUT_DIR@$newdir/dsout/@" $newdir/runcode
