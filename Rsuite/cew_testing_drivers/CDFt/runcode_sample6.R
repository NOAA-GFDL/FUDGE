...
#        ds.method <- 'CDFt'
        #ds.method <- 'DeltaSD'
ds.method <- 'CGDS'
	ds.experiment <- 'test2-CGDS-A38e-mL01K00' 
	k.fold <- 0 
...
args=list(deltaop='add', deltatype='mean')
...
output.file <- '/home/cew/testout.txt'
Rprof(output.file, memory.profiling=TRUE, append=FALSE)
source(paste(FUDGEROOT,'Rsuite/Drivers/','Master_Driver.R',sep=''))
Rprof(NULL)
print("Summary of calls by memory use and time:")
profile <- summaryRprof(output.file, memory = "stats", chunksize=5000) #memory='both' or 'stats' tseries?
print(profile)
