...
#------------- method name k-fold specs-----------------------#
        #ds.method <- 'DeltaSD' 
  ds.method <- "CDFt"
	ds.experiment <- 'PMprp1-DeltaSD-A18-pr_mod_test' 
	k.fold <- 0 
	pr_opts=list(pr_threshold_in='us_trace',pr_freqadj_in='on',pr_conserve_in='on',pr_threshold_out='us_trace',pr_conserve_out='on')
#-------------- output -----------------------#
	output.dir <- '/home/cew/Code/testing/'
	mask.output.dir <- '/home/cew/Code/testing/' 
#-------------  custom -----------------------#
        #args=list(deltaop='ratio',deltatype='median') 
...
post_ds = list(mask1=list(type='PR', qc.mask='off', adjust.out='on', loc='outloop', 
                          qc_args=list(thold='us_trace', conserve='on'))) 
