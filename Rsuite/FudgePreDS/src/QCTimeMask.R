...
QCTimeMask <- function(hist.train.mask, hist.targ.mask, esd.gen.mask, k=0, method="generic"){
  ...
  if ( (t.pred.masks$dim$time[1] != t.targ.masks$dim$time[1]) ||
      (t.pred.masks$dim$time[length(t.pred.masks$dim$time)] != t.targ.masks$dim$time[length(t.targ.masks$dim$time)]) ){
    stop(paste("Training period time error: The start and end dates of the training target", 
               t.targ.masks$dim$time[1], t.targ.masks$dim$time[length(t.targ.masks$dim$time)],
               "are not the same as the start and end dates of the training predictor,", 
               t.pred.masks$dim$time[1], t.pred.masks$dim$time[length(t.pred.masks$dim$time)]))
  }
  #When k > 1, both training and esdgen will have the same length
  #and start/end date
  message("Passing k > 1 time comparison")
  if (k > 1){
    if ( (t.pred.masks$dim$time[1] != esd.gen.masks$dim$time[1]) ||
           (t.pred.masks$dim$time[length(t.pred.masks$dim$time)] != esd.gen.masks$dim$time[length(esd.gen.masks$dim$time)]) ){
      stop(paste("K > 1 Time Period Error: The start and end dates of the training period", 
                 esd.gen.masks$dim$time[1], esd.gen.masks$dim$time[length(esd.gen.masks$dim$time)],
  ...
  if (length(t.pred.masks$masks)!= length(t.targ.masks$masks) || 
        length(t.targ.masks$masks)!= length(esd.gen.masks$masks)){
    stop(paste("Time mask dimension error: time mask files are expected to have the", 
               ...
               length(t.pred.masks$masks), ",", hist.targ.mask, "had",
               length(t.targ.masks$masks), ",", "and", esd.gen.mask, "had", 
               length(esd.gen.masks$masks)))
  }
