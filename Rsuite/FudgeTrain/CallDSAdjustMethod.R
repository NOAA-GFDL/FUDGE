...
#'@param adjust.method: The DS adjustment value being used.
#'May use the downscaled data, the qc mask, or many other possible values.
#'@param args: The arguments to the ds value adjustment function. Should 
#'probably, at some point in the future, have a separate check implemented
#'in here. 
#'@param data: The downscaling data to be processed. Should neve be NULL. 
#'
#'----Arguments that should be used, like 90% of the time----
#'@param qc.mask: A mask of QC values, presumably where 1=check passed
#'and 0=check failed. NA values mean that there was missing data
#'in the calculation (and therefore, presumably, no calculation neccesary)
#'
#'----Arguments that *may* be used, depending-----
#'Some arguments may or may not be used, depending on what data is available
#'and how the function is being called. Refer back to this for more info
#'on a later date.
#'@param hist.pred
#'@param hist.targ
#'@param hist.fut
#'
#'
#'@return A vector of adjusted values of the same length as the data parameter
#'@author Carolyn Whitlock, October 2014
#'

CallPostProcMethod <- function(adjust.method, args=NULL, data, 
                               mask, 
                               hist.pred=NA, hist.targ=NA, hist.fut=NA){
  switch(adjust.method, 
         'compare.correct' = adjust_byCompare(data, mask, args, hist.pred, hist.targ, hist.fut),
         'Nothing' = adjust_Nothing(data, args), 
         ReturnPostProcMethodError(pp.method))
}

ReturnPostProcMethodError <- function(adjust.method){
  stop(paste("DS Adjustment Method Error: method", adjust.method, 
...
adjust_byCompare <- function(data, mask, args, hist.pred, hist.targ, hist.fut){
  #TODO: At some point, include the var post-processing option
  #and some sort of units check to go with it.
  #Is it worth trying to include the units in the args list for this
  #particular function? Or should it just be included in all functions where
  #it's expected that there might be a comparison against a hard-coded constant?
...
ncks.clone <- function(old.file, var, new.file, force_override=TRUE){
  ...
  #in general and the nco utils in particular
