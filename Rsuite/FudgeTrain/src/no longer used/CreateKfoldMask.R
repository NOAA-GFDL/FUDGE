...
#' 
#' 
#' Note: was in the process of moditying to do the crossvalidation by year
#' properly. Got distracted and told that this might not be all that important.
#' Left it to work on later.


CreateKfoldMask<-function(timeseries, vector.length, k, byyear = TRUE){
  if (byyear){
    #If making sure that all kfold masks separate cleanly by year
    year.factor <- as.factor(strftime(timeseries,"%Y"))
    #Then the number of years is the number you want to calculate on, 
    #no the total number of days
